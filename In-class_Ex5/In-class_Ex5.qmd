...
Spatial Interaction Models are often used to explain origin-destination (OD) flows that arise in fields such as public bus commuting. These models rely on a function of the distance between the origin and destination as well as explanatory variables pertaining to characteristics of both origin and destination locations. Spatial interaction models assume that using distance as an explanatory variable will eradicate the spatial dependence among the sample of OD flows between pairs of locations. The notion that distance functions in conventional spatial interaction models effectively captures spatial dependence in interregional flows has long been challenged. In view of the limitation Spatial Interaction Models to account for spatial dependence, Spatial Econometric Interaction Models have been introduced James P. LeSage and R. Kelley Pace (2009).

In this in-class exercise, I will gain hands-on exercise on using spflow package, a R library specially developed for calibrating Spatial Econometric Interaction Models. By the end of this in-class exercise, I will acquire the skills to:
...
-   integrate these explanatory variables into a tidy variables tibble data.frame.
...
| This will install the development version of **spflow** package.
...
Before we can calibrate Spatial Econometric Interaction Models by using **spflow** package, three data sets are used. They are:

-   spatial weights

-   a tibble data frame consisting of the origins, destination, flows and distances between the origins and destination,

-   and a tibble data frame consisting of the explanatory variables.
...
In the code chunk below, *MPSZ-2019* shapefile will be imported into R environment as a sf tibble data.frame called *mpsz*.
...
We now save `mpsz_nb` into an **rds** file for subsequent use by using the code chunk below.
...
In this section, I will learn how to prepare a flow data at the planning subzone level as shown in the screenshot below.
...
The `flow_data` should look similar to the table below.
...
Let us load in the datasets below.
...
mpsz_net = spflow_network(
  #id_net can be assigned an arbitrary value
  id_net = "sg",
  node_neighborhood = 
...
In **spflow** package, we can use [`spflow_network_pair()`](https://lukece.github.io/spflow/reference/spflow_network_pair.html)
...
We now calibrate a base model with the following configuration:
